# 2022 Innovation Camp Coding Challenge Solutions
...
library(dplyr) # allows data manipulation
...
```{r}
df <- df %>% select(-("SCALAR_FACTOR" | "SCALAR_ID" | "DECIMALS"))
# ALTERNATIVELY: df <- df %>% select(-("DECIMALS | starts_with("SCALAR")))
```
...
# EXAMPLE SOLUTION
#Check the variety of values in GEO and Stocks
print(unique(df["GEO"]))
print(unique(df["Stocks"]))
...
fig <- plot_ly(
  ...
  layout(title="Creamery Butter Stock in Canada", xaxis = list(title = "Date"), yaxis = list(title = "Value (t)")) # set the title for the graph and axes.
...
```{r}
#Let's do variety cheese in Alberta, for example.

#create a bar plot
fig <- plot_ly(
  data = df %>% filter(GEO == "Alberta" & Commodity == "Variety cheese" & Stocks == "Total stocks"), # filtering for Alberta, Variety cheese, and total stock values only
  x = ~Date, # the x-axis will be the date
  y = ~VALUE, # the y-axis will be the values of the commodity
  type = "bar" # the type of plot will be bar
  ) %>%
  layout(title="Variety cheese Stock in Alberta", xaxis = list(title = "Date"), yaxis = list(title = "Value (t)")) # set the title for the graph and axes.
...
```{r}
# create a pie chart showing the composition of total dairy stocks in Canada in March 1974
fig <- plot_ly(
  data = df %>% filter(GEO == "Canada" & Date == "1974-03-01" & Stocks == "Total stocks"), # filter for Canada, total stocks and March 1974 only
  labels = ~Commodity, # the labels of the pie chart will be the commodities
  values = ~VALUE, # the sizes of each slice are determined by the values of commodities
  type = "pie" # the plot will be a pie chart
  ) %>%
  layout(title="Canadian Dairy Product Stocks for 1974-03-01") # set the chart name
...
```{r}
# let's re-visualize the above pie charts as a bar chart

fig <- plot_ly(
  data = df %>% filter(GEO == "Canada" & Stocks == "Total stocks" & Date == "1974-03-01"), 
  x = ~Commodity,
  y = ~VALUE, 
  type = 'bar')%>%
  layout(title="Canadian Dairy Product Stocks for 1974-03-01", xaxis = list(title = "Commodity"), yaxis = list(title = "Value")) # set the title for the graph and axes.
...
```
Let's visualize this! Note that in the above code, we stored the filtered result under the variable "data" so we can refer to it later on.
...
data = df %>% 
    filter(Stocks == "Total stocks" & GEO == "Canada") %>%
    group_by(GEO, Commodity, year = lubridate::floor_date(Date, "year")) %>% # group the df by region and Commodity for each year
    summarize(mean = mean(VALUE, na.rm=TRUE)) # take the average of the values in each group we made above
...
```{r}
dfpop = get_cansim("17-10-0005-01") # load population data

```

```{r}
names(dfpop)[names(dfpop) == "Age group"] <- "Age" # one of the columns has a space in it, this isn't good! remove it!
...
```{r}
dfpop <- dfpop %>%
  filter(GEO == "Canada" & Sex == "Both sexes" & Age == "All ages") # filter the population data for Canada, both sexes, and all ages to get the appropriate total
...
data <- data[-c(1,53),] # the dairy data has data for 1970 and 2022, but the population data does not. Let's drop the first and last row.
...
data$percapita <- data$mean*1000/dfpop$VALUE 
...
fig <- plot_ly(
  data, 
  x = ~year, # the x-axis will be the year
  y = ~percapita, # the y-axis will be the yearly mean values of the commodity percapita
  type = "bar" # the type of plot will be bar
  ) %>%
  layout(title="Cheddar Cheese Stock Per Capita in Canada", xaxis = list(title = "Year"), yaxis = list(title = "kg")) # set the title for the graph and axes.

fig # call the fig to display it

```
Now let's try for each province.

```{r}
# Let's check the provinces

# initial filters stay the same
data = df %>% 
    filter(Stocks == "Total stocks") %>%
    group_by(GEO, Commodity, year = lubridate::floor_date(Date, "year")) %>% # group the df by region and Commodity for each year
    summarize(mean = mean(VALUE, na.rm=TRUE, .groups = 'keep')) # take the average of the values in each group we made above

data <- data[-c(1,53),] # the dairy data has data for 1970 and 2022, but the population data does not. Let's drop the first and last row.
data <- data %>%filter(Commodity == "Cheddar cheese") 

# our list of provinces from the original dataset
provinces <- unique(df["GEO"])

# some are missing too much data, let's drop them
remove <- c("Maritime provinces","Prince Edward Island","Nova Scotia","New Brunswick","Saskatchewan","Other Provinces","Atlantic provinces")
provinces <- provinces[!(provinces$GEO %in% remove),]

# loop over the provinces 
for (i in 1:nrow(provinces))
{
  datatemp <- filter(data, GEO == provinces[i, "GEO"]) # filter for province
# create a new column called percapita equal to the mean times 1000 (to get kg) divided by the population to get average kg of cheese per person
datatemp$percapita <- datatemp$mean*1000/dfpop$VALUE 
...
fig <- plot_ly(
  ...
  layout(title=paste("Cheddar Cheese Stock Per Capita in", provinces[i, "GEO"]), xaxis = list(title = "Year"), yaxis = list(title = "kg")) # set the title for the graph and axes.
print(fig) # call the fig to display it
} # end loop
...
fig <- plot_ly(
  ...
  layout(title="Cheddar Cheese Stock Per Capita in Ontario", xaxis = list(title = "Year"), yaxis = list(title = "kg")) # set the title for the graph and axes.
